...
def load_data():
    possible_paths = [
        "data/HHS_Data_Unaccompanied_Alien_Children_Program",
        "../data/HHS_Data_Unaccompanied_Alien_Children_Program",
        "HHS_Data_Unaccompanied_Alien_Children_Program",
...
# SECTION 9: EXECUTIVE BRIEFING FOR GOVERNMENT STAKEHOLDERS

st.markdown("---")
st.markdown('<div class="section-title">🏛️ Executive Briefing for Government Stakeholders</div>', unsafe_allow_html=True)
st.markdown('<div class="section-subtitle">Policy-aligned summary for federal decision-makers and program administrators</div>', unsafe_allow_html=True)

# Calculate policy-relevant metrics
avg_daily_intake = filtered_df["apprehended"].mean()
avg_daily_transfers = filtered_df["transferred"].mean()
avg_daily_discharges = filtered_df["discharged"].mean()
peak_load_date = filtered_df.loc[filtered_df["Total_System_Load"].idxmax(), "date"]
peak_load_value = filtered_df["Total_System_Load"].max()
total_stress_days = int(filtered_df["Stress_Flag"].sum())
avg_cbp_load = filtered_df["in_cbp"].mean()
avg_hhs_load = filtered_df["in_hhs"].mean()
cbp_hhs_ratio = avg_cbp_load / avg_hhs_load if avg_hhs_load > 0 else 0

# Determine policy status
if total_stress_days > 30:
    policy_status = "HIGH ALERT"
    policy_color = "#EF4444"
    policy_bg = "rgba(239, 68, 68, 0.15)"
elif total_stress_days > 10:
    policy_status = "ELEVATED ATTENTION"
    policy_color = "#F59E0B"
    policy_bg = "rgba(245, 158, 11, 0.15)"
else:
    policy_status = "NORMAL OPERATIONS"
    policy_color = "#10B981"
    policy_bg = "rgba(16, 185, 129, 0.15)"

# Main container with colored border
st.markdown(f"""
<div style="background: {policy_bg}; border: 2px solid {policy_color}; border-radius: 16px; padding: 1.5rem; margin: 1rem 0;">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
        <div>
            <h3 style="color: {policy_color}; margin: 0; font-size: 1.4rem;">📋 Federal Program Status: {policy_status}</h3>
            <p style="color: #94A3B8; margin: 0.3rem 0 0 0; font-size: 0.85rem;">Reporting Period: {start_date} to {end_date}</p>
        </div>
        <div style="text-align: right; margin-top: 0.5rem;">
            <div style="font-size: 2.2rem; font-weight: 800; color: {policy_color};">{total_load:,}</div>
            <div style="color: #94A3B8; font-size: 0.8rem;">Children Under Federal Care</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with met1:
    st.metric("Avg Daily Intake (CBP)", f"{avg_daily_intake:.0f}")
with met2:
    st.metric("Avg Daily Transfers (HHS)", f"{avg_daily_transfers:.0f}")
with met3:
    st.metric("Avg Daily Discharges", f"{avg_daily_discharges:.0f}")
...
insights_text = f"""
- Peak system load of **{peak_load_value:,.0f} children** recorded on **{peak_load_date.strftime('%B %d, %Y')}**
- **{total_stress_days}** days classified as high-stress periods requiring federal attention
- CBP-to-HHS load ratio: **{cbp_hhs_ratio:.2f}** — {'CBP bottleneck detected' if cbp_hhs_ratio > 0.3 else 'Balanced flow between agencies'}
- Current backlog trend: **{backlog_status}** ({backlog_trend:+.0f} over 14 days)
"""
...
if policy_status == "HIGH ALERT":
    rec_text = "🚨 **URGENT:** Sustained capacity strain detected. Recommend immediate activation of emergency shelter protocols and accelerated sponsor vetting procedures."
    rec_color = "#EF4444"
elif policy_status == "ELEVATED ATTENTION":
    rec_text = "⚠️ **ATTENTION:** Elevated stress periods observed. Recommend increased inter-agency coordination and proactive resource allocation for upcoming high-demand periods."
    rec_color = "#F59E0B"
else:
    rec_text = "✅ **STABLE:** System operating within sustainable parameters. Continue current operational standards and maintain readiness protocols."
    rec_color = "#10B981"

st.markdown(f"""
<div style="background: rgba(15, 23, 42, 0.6); border-left: 4px solid {rec_color}; padding: 1rem 1.2rem; border-radius: 8px;">
    <p style="color: #E2E8F0; margin: 0; line-height: 1.6;">{rec_text}</p>
</div>
""", unsafe_allow_html=True)



# SECTION 10: EXECUTIVE INSIGHTS & RECOMMENDATIONS
...
# SECTION 12: PDF REPORT GENERATOR (FIXED)
